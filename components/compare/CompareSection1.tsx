...
// Adjusted mockDataChart1 to match API response (excluding heart disease for now)
const mockDataChart1 = [
  { id: 2, name: "โรคความดันโลหิตสูง", color: "#1E4D91" },
  { id: 3, name: "โรคเบาหวาน", color: "#FFC300" },
  { id: 4, name: "โรคซึมเศร้า", color: "#6A0572" },
];
...
const CompareSection1 = () => {
  ...
  const [comparisonData, setComparisonData] = useState<DiseaseData[] | null>(null);
      ...
      <Box display="flex" height="100%" gap="24px" minHeight="362px">
        <Box display="flex" width="100%" className={styles.boxChart}>
          <LineChart
            data={comparisonData}
            chartType="disease"
            timePeriod={(rangeDate?.value || "Yearly") as "Yearly" | "Monthly" | "Daily"}
          />
